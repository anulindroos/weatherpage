// Tätä viikkoa varten, asenna curl ( ei väliä mikä kansio, vaikka viikkoharjoituksissa)
// komennolla: sudo apt install curl
// backend kansiossa aja käsky: npm install koa-bodyparser
// sen jälkeen voit joko käyttää tätä tiedostoa, tai muokata omaasi
// pyytäkää jos en oo vielä ehtinyt tätä laittaa saataville
...
test.get(`${apiPath}/weather/temperature`, async (ctx) => {
  try {
  
    const [data] = await pool.execute(`
        SELECT *
        FROM weather WHERE (data -> '$.AAM') IS NOT NULL LIMIT 50
      `);
